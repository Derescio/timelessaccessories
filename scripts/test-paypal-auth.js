...
async function getAccessToken() {
  try {
    if (!CLIENT_ID || !CLIENT_SECRET) {
      throw new Error(
        "PayPal CLIENT_ID and CLIENT_SECRET must be configured in environment variables"
      );
    }

    console.log("Using PayPal API URL:", PAYPAL_API_BASE);
    console.log(
      "Using CLIENT_ID starting with:",
      CLIENT_ID.substring(0, 5) + "*".repeat(CLIENT_ID.length - 5)
    );
...
(async () => {
  ...
  try {
    ...
    console.log(
      "Token received successfully (first 5 chars):",
      token.substring(0, 5) + "*".repeat(10)
    );
    ...
    if (verifyResponse.ok) {
      ...
      console.log(
        "Token verification successful! API response:",
